...
using static Vim.G3dNext.Tests.TestUtils;
...
{
    [Test]
    public static void FileRepackage()
    {
        //var name = "_MAIN-AEI_HSIB-R20.v1.2.73";
        var name = "residence";
        //var name = "tower";
        var path = $"./{name}.vim";

        FileUtils.SplitVim(path, name);
    }

    [Test]
    public static async Task Filter()
    {
        var file = await DownloadFileAsync("https://vimdevelopment01storage.blob.core.windows.net/samples/residence_nozip.vim");
        var options = new DataFormat.LoadOptions()
        {
            SkipAssets = true,
            SkipEntities = true,
            SkipHeader = true,
        ...
        var vim = VimScene.LoadVim(file, options);
        var bytes = vim.Document.Geometry.WriteToBytes();
        G3DNext<VimAttributeCollection>.TryRead(bytes, out var g3d);
        var next = new VimG3d(g3d);
        var instances = Enumerable.Range(1, 1000).ToArray();
        var filtered = next.Filter(instances);
    }
    ...
    public static void TriangleTest()
    {
        ...
        if (!G3DNext<VimAttributeCollection>.TryRead(bytes, out var g))
    ...
    public static void UnexpectedG3dTest()
    {
        ...
        var result = G3DNext<VimAttributeCollection>.TryRead(stream, out var g3d);
    ...
    public static void ReadG3dInVimTest()
    {
        ...
        G3DNext<VimAttributeCollection>? g3d = null;
        stream.ReadBFast<object?>((s, name, size) =>
        {
            if (name == "geometry")
                G3DNext<VimAttributeCollection>.TryRead(s, out g3d);
            else
    ...
    public static void WriteG3dTest()
    {
        ...
        var readResult = G3DNext<VimAttributeCollection>.TryRead(g3dFilePath, out var readG3d);
