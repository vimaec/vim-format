...
namespace Vim.G3dNext.Attributes
{
    ...
    {
        public static int GetMeshSubmeshStart(this VimAttributeCollection c, int mesh)
        {
            return c.MeshSubmeshOffsetAttribute.TypedData[mesh];
        }

        public static int GetMeshSubmeshEnd(this VimAttributeCollection c, int mesh)
        {
            return mesh + 1 < c.GetMeshCount()
                ? c.MeshSubmeshOffsetAttribute.TypedData[mesh +1]
                : c.GetSubmeshCount();
        }

        public static int GetMeshSubmeshCount(this VimAttributeCollection c, int mesh)
        {
            return c.GetMeshSubmeshEnd(mesh) - c.GetMeshSubmeshStart(mesh);
        }
